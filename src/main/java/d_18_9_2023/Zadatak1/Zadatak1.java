...
public class Zadatak1 {
    public static void main(String[] args) {
        VideoPlayer videoPlayer = new VideoPlayer(250, 120, 30);

        videoPlayer.print();

        TimeControl timeControlIncrease = new TimeControl(true);
        timeControlIncrease.doAction(videoPlayer);
        timeControlIncrease.doAction(videoPlayer);
        timeControlIncrease.doAction(videoPlayer);

        TimeControl timeControlDecrease = new TimeControl(false);
        timeControlDecrease.doAction(videoPlayer);

        AudioControl audioControlIncrease = new AudioControl(true);
        audioControlIncrease.doAction(videoPlayer);
        audioControlIncrease.doAction(videoPlayer);
        audioControlIncrease.doAction(videoPlayer);

        AudioControl audioControlDecrease = new AudioControl(false);
        audioControlDecrease.doAction(videoPlayer);

        QualityOptimizerControl qualityOptimizerControl = new QualityOptimizerControl(150);
        qualityOptimizerControl.doAction(videoPlayer);
