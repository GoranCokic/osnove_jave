...
public class Zadatak4 {
    public static void main(String[] args) {
        String locustKomanda = "locust -f $LOCUSTFILE -u $NUMBER_OF_USERS -r $SPAWN_RATE -t $RUN_TIME --headless –host $HOST --html $HTML_FILE";
        int brojKomandiZaZamenu = brojiKomandeZaZamenu(locustKomanda);

        Scanner s = new Scanner(System.in);

        for (int i = 0; i < brojKomandiZaZamenu; i++) {
            System.out.println(locustKomanda);
            System.out.print("Unesite naziv promenljive za zamenu: ");
            String zaPromenu = s.next();
            System.out.println();
            System.out.print("Unesite novu komandu: ");
            String promena = s.next();
            locustKomanda = locustKomanda.replace(zaPromenu, promena);
            System.out.println();
    ...
    public static Integer brojiKomandeZaZamenu(String someString) {
        int counter = 0;
